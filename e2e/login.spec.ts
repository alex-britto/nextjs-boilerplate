...
test('invalid email', async ({ page }) => {
    ...
    await expect(page.getByTestId('email-error')).toContainText('Insira um e-mail válido')
})
...
test('invalid password', async ({ page }) => {
    ...
    await page.getByRole('button').click();
    await expect(page.getByTestId('password-error')).toContainText('A senha deve ter pelo menos 6 caracteres')
})
